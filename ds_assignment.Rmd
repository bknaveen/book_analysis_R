...
cat(sum(is.na(rating_df)))
```

```{r }
#check null value of user data-frame
cat(sum(is.na(user_df)))
```
...
merged_df <- merge(rating_df, user_df, by = "User.ID")
# create Age Groups
merged_df$AgeGroup <- cut( #where cuts divided numeric vectors in intevals
  merged_df$Age,
  breaks = c(-Inf, 18, 30, 45, 60, Inf),
  labels = c("0-18", "19-30", "31-45", "46-60", "61+"),
  right = TRUE
)
# display top 5 merged data frames.
...
set.seed(123) 
# Sample 500 rows from merged_df (only rows with non-NA AgeGroup)
...
avg_rating_PGB <- RatingPGB %>%
  ...
  summarise(AvgRating = mean(Book.Rating, na.rm = TRUE), Count = n()) %>%
  filter(Count >= 10)
#print top 5 rows of average RatingPGA
head(avg_rating_PGA)
```
# top five rows for ratingPGB
```{r}
#print top 5 rows of average RatingPGB
head(avg_rating_PGB)
```
